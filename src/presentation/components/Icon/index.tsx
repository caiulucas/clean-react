import React from 'react';

import styles from './styles.scss';

type Props = {
  iconName: 'thumbDown' | 'thumbUp';
  className?: string;
};

export const icons = {
  thumbDown: {
    src: 'data:image/png;base64,',
    alt: 'Thumb down image',
  },
  thumbUp: {
    src: 'data:image/png;base64,',
    alt: 'Thumb up image',
  },
};

export function Icon({ iconName, className }: Props) {
  const iconStatus = iconName === 'thumbDown' ? styles.invalid : styles.valid;

  return (
    <div className={[styles.iconWrap, iconStatus, className].join(' ')}>
      <img src={icons[iconName].src} alt={icons[iconName].alt} role="icon" />
    </div>
  );
}
